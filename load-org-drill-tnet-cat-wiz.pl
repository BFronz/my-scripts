...
$file      = "OrgVisitsCatDrillCovNewReg";  # new reg data file
...
while (!eof(rf))
  {
   ...
   {  
    ...
    if($heading ne 0 && $org ne "?"  &&  $org ne ""  &&  $org ne "#NAME?"  &&  $org ne "unknown"  &&  $org ne "-"  && $heading>0 && $org ne "no company") {
    #if($instr !~ /[^\x00-\x7e]/) {

     if($block ne "Y") {     		
     print wf "$date\t$acct\t$heading\t$org\t$dom\t$country\t$city\t$state\t$zip\t$cnt\t$isp\t$block\t$orgid\t$orgmd5\t$ip\t$naics\t$primary_sic\t$countrycode\t$duns_number\t$domestichqdunsnumber\t$hqdunsnumber\t$gltdunsnumber\t$countrycode3\t$audience\t$audiencesegment\t$b2b\t$employee_range\t$forbes2k\t$fortune1k\t$industry\t$informationlevel\t$latitude\t$longitude\t$phone\t$revenue_range\t$subindustry\t$cov\t$zipcov{$zip}\n";
     }
      #} 
    }

    $date = $heading = $org = $dom = $country = $city = $state = $zip = $cnt =  $orgid = $ip = $isp = $naics = $primary_sic = $countrycode = $orgmd5  = $zipcheck="";

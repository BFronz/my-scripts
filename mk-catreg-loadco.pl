...
# Get company words from catreg_history
$i=0;   
$query  = " select trim(company),tinid from  catreg_history where length(company)>1  ";
#$query  = " select trim(company),tinid from  catreg_history where length(company)>1 and origin='TNET' $limitq ";

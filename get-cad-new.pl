...
# nohup ./get-cad-new.pl 11/01/2016 11/30/2016 &
# nohup ./get-cad-new.pl 08/01/2017 08/31/2017 &
...
$cmd     = "wget $user $timeout \"$BASEURL\" -O /usr/wt/$proc";
system("$cmd");
...
while (!eof(INFILE)  ) {
  ...
  $count++;   
  $line = <INFILE>;
  if(!$line =~ /<Company /) { next; }
  if ($line =~ / CID=\"([^\"]+)\"/)         { $cid = $1; }
  if ($line =~ / TGRAMSID=\"([^\"]+)\"/)    { $tnet = $1;  }
  if ($line =~ / isFCI=\"([^\"]+)\"/)       { $isFCI = $1;  }
  #print "$cid \t $tnet\n";
  if($cid ne "" && $tnet ne ""  && $isFCI eq "true") 
    { 
     $tnetmatch{$cid} = "$tnet"; 
     $allcids .= "$cid,";
     $match++;
...
close(OUTFILE);
print "total $count | cid to tgrams $match\n";
chop ($allcids);
...
$timeout = " --timeout=14400 --tries=1  ";
$cmd     = "wget $user $timeout \"$BASEURL?$params\" -O  /usr/wt/$proc"; 
#print "\n$cmd\n";
system("$cmd");

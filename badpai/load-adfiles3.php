...
while (!feof($rf)) 
{
	...
	{ 
		...
		if(mysql_result($result, 0, "informationlevel") == "basic")
		{
			...
			$statename            = GetState(mysql_result($nresult, 0, "regioncode"));
			$state                = mysql_result($nresult, 0, "region"); // 2 char state, so no lookup
			$state                = PrepData($state);
			$zip                  = PrepData(mysql_result($nresult, 0, "postalcode"));
			//$country              = GetCountry (mysql_result($nresult, 0, "country"))
			$country              = mysql_result($nresult, 0, "twolettercountry");	// 2 char state, so no lookup

...
$file      = "orgDomainVisitsDrillCov";
$file      = "advOrgVisitsDrillcovnewregnoCID";
$infile    = $fyear . "/" . $file . "-" . $fdate . ".txt";

$file2      = "orgDomainVisitsDrill"; 
$file2      = "org_DomainVisitsDrillReg"; 
$infile2    = $fyear . "/" . $file2 . "-" . $fdate . ".txt";
...
close(rf);
close(wf);
system("mysqlimport -iL thomas $DIR/tnetlogORGSITED_temp.txt");    
#system("mysqlimport thomas $tablefile");   

# load temp table 
open(wf, ">tnetlogORGSITED_temp.txt")  || die (print "Could not open tnetlogORGSITED_temp.txt\n");
print "$infile2\n";  
open(rf, "$infile2")  || die (print "Could not open $infile2\n");
while (!eof(rf))
  {
    $instr = <rf>;
    chop($instr); 

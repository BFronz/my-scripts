...
define ("NA_ORGANIZATION_DB", 13); //new
...
//
// NetAcuityOrganization - Query the company database.
//
class NetAcuityOrganization extends NetAcuity
{
    var $organization;

    function NetAcuityOrganization()
    {
        $this->setVariables();
    }
    ...
    function parse()
    {
        if(($this->organization = strtok($this->raw_response, ";")) == null)
        {
            return 0;
        }
    ...
    {
        $this->setVariables();
        $ret = $this->na_query_raw($queryIP, NA_ORGANIZATION_DB);
        if ((!$ret) || ($this->response_size == 0))
        {
            return 0;
        }

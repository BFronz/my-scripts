...
#
#
# Matches bad words/phrases in catreg_history with mt_badwords. 
...
$query  = " select word from mt_badword  ";
...
# Loop through catreg_history table (fname,lname,company,zip fields) and loop through word array. 
...
$query  = " select fname, lname, company, zip, catreg_history.tinid from catreg_history ";
$query .= " LEFT JOIN mt_ringer ON  catreg_history.tinid=mt_ringer.tinid ";
$query .= " where mt_ringer.tinid is null ";    
#$query .= " limit 10  "; 
...
system("mysqlimport -i tgrams $writefile");

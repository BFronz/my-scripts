...
$infile = "visitor_cat_tool-test.txt";
$infile = "visitor_cat_tool.bak";
   
$outfilen = "visitor_cat_tool";
...
while (!eof(rf))
{
 $line = <rf>;
 chop($line);
 ($heading, $org, $domain, $city, $state, $zip, $isp, $block, $ocleanname, $dcleanname, $orgid, $oid, $checked) = split(/\t/, $line); 
 ...
 if($stop eq "N")
 {                  
  print wf "$heading\t$org\t$domain\t$city\t$state\t$zip\t$ocleanname\t$dcleanname\t$orgid\t$oid\t$checked\t$ip\t$naics\t$primary_sic\t$countrycode\t$country\n";
 }  
 print "$i\t$org\t$stop\n";

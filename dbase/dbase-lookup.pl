...
my $outfile  = "dbaseIPresolved.txt";
open(wf,  ">$DIR/dbase/$outfile")  || die (print "Could not open $outfile\n");

my $badfile  = "dbaseIPbad.txt";
open(wf2,  ">$DIR/dbase/$badfile")  || die (print "Could not open $badfile\n");
...
$query = "SELECT * FROM dbaseNetAcuityIP WHERE  ip='42.99.164.64'  ";
#$query = "SELECT * FROM dbaseNetAcuityIP WHERE  ip='112.215.11.210'  ";
#$query = "SELECT ip FROM dbaseIPresolved WHERE informationlevel!='detail' limit 100,1 ";

$query = "SELECT * FROM thomdbaseNetAcuityIP WHERE  created=0"; 
$query = "SELECT ip FROM dbaseIPresolved WHERE isp='true' limit 20 ";
#print "\n$query\n";         
...
{   
	...
	# retreive data, print out 404 urls
	 ...
	 # gets not working trying curl
	 my $json = qx{curl --silent $dburl};    #  || $badrec++;
	...
	# write out
	print wf "$ip\t$created\t$org\t$dom\t$city\t$state\t$zip\t$country\t$isp\t$naics\t$primary_sic\t$dunsnum\t$domestichqdunsnumber\t$hqdunsnumber\t$gltdunsnumber\t$countrycode\t$countrycode3\t$audience\t$audiencesegment\t$b2b\t$employee_range\t$forbes2k\t$fortune1k\t$industry\t$informationlevel\t$latitude\t$longitude\t$phone\t$revenue_range\t$subindustry\n";
	}    

...
# Connect to mysql database for tgrams query
$db   = "tgrams";
$data_source = "";
$user = "";
$auth = "";
...
$query = " update contacts set ccovarea='NA' where ccovarea='' "; 
...
$query = " select acct, heading,ccovarea, count(*) from contacts where (created>=$start and created<=$end) and  notsent < 1  ";
...
$query = " select acct, heading, count(*) from contacts where (created>=$start and created<=$end) and  notsent < 1  ";

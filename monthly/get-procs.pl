...
###>> not used if($extra) {$outfile="reports/$proc-$fdate-$extra.txt";}
   
$BASEURL = "http://65.215.104.135/cgi-bin/xmlgrab.pl";
$BASEURL = "http://wtdata.tnt.com/cgi-bin/xmlgrab.pl/cgi-bin/xmlgrab.pl";
$BASEURL = "http://thomasxml.insightrocket.com/xmlgrab.pl";
$BASEURL = "http://thomasxml.insightrocket.com/xmlgrab2.pl";  # new per David M. May 2012
$params  = "site=$wtsite"; 
$params .= "&start=$Smonth/$Sday/$Syear&end=$Emonth/$Eday/$Eyear";
if($dset ne 0)      { $params .= "&dset=$dset"; }  
if($group ne 0)     { $params .= "&group=$group"; }  
if($nat ne 0)       { $params .= "&nat=$nat"; } else { $params .= "&nat=0"; }
if($origin ne "")   { $params .= "&$origin"; }  # format in string  
if($exclude ne "")  { $params .= "&exclude"; }  # another strange var 
$params .= "&fmt=1";
print "$proc Proc:\t$BASEURL?$params\n";   
            
#open(wf ,">>procs-aug-2014.sh") || die "not found";
#print wf "$proc\t$BASEURL?$params\n"; 
#print wf "curl -sS  \"$BASEURL?$params\" -o $proc-$fdate.xml\n"; 
...
while (!eof(INFILE)  ) {
  ...
  if($line =~ /dbi:null-columns/) { print OUTFILE2 "$str\n"; next; } # null tinid values
  ...
  $conversions = $visits = $visitors = $pgspervisit = $avgminutes = $printcat = $adov = $frequency = $rfq = $CADDownloads  =  $pv = "";

...
while (!eof(rf))
  {
   ...
   if($org  =~ /fuck/ || $org =~ /asshole/)  { $block="Y"; } 
    ...
    $block=$b2b=$employee_range=$forbes2k=$fortune1k=$industry=$informationlevel=$latitude=$longitude=$phone=$revenue_range=$subindustry=$tinid=$tinidok="";
...
system("mysqlimport -L thomas $DIR/newsORGSITED$yy.txt");
...
#print "Done, remember to uncomment\n"; exit; 
...
system("mysqlimport -L thomas  $DIR/newsORGD$yy.txt");

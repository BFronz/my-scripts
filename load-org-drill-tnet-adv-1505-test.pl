...
open(wf2,  ">>tnetlogORGD15_05_ISP_conversions.txt")  || die (print "Could not open tnetlogORGD15_05_ISP_conversions.txt\n");

open(wf3,  ">>tnetlogORGD15_05_regs.txt")  || die (print "Could not open tnetlogORGD15_05_ISP_regs.txt\n");
...
while (!eof(rf))
  {
	...
	if($conversion eq "Y" && $isp eq "N" && $org ne "") {print wf2 "$orgorig\t$org\n";} 


   if($acct ne 0  && $org ne "?"  &&  $org ne ""  &&  $org ne "#NAME?"  &&  $org ne "unknown"  &&  $org ne "-"  && $org ne "no company" ) {
  
     if($block ne "Y"){ print wf3 "$orgorig\t$org\n"; }
    } 

...
$table     = "tnetlogORGSITEREG" . $yy . "W";
$tablefile = "tnetlogORGSITEREG" . $yy . "W.txt";
...
# pull data           0         1      2          3        4        5            6            7       8
$query  = "SELECT m.company, m.city, m.state, m.country, m.zip, m.industry, m.jobfunction, m.phone, count(*) ";
$query .= "FROM   tgrams.mt_profile_history as m, $intable as r   ";
$query .= "WHERE  m.tinid=r.tinid  AND date='$fdate' ";
$query .= "GROUP  by r.tinid  ";  
...
while (my $row = $sth->fetchrow_arrayref)
 {  
    ...
    $orgid  = "$fdate$org$dom$countrycode$city$state$zip$cov";                                        
    $orgid  =  md5_hex($orgid); 
    $orgmd5 =  md5_hex($org); 

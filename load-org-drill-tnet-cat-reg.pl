...
$yy        = substr($fdate, 0, 2);             
$mm        = substr($fdate, 2, 2);
$intable   = "tnetlogREGCAT" . $yy;
$table     = "tnetlogORGCATREG" . $yy . "W";
$tablefile = "tnetlogORGCATREG" . $yy . "W.txt";
...
$query  = "SELECT m.company, m.city, m.state, m.country, m.zip, m.industry, m.jobfunction, m.phone, count(*), r.acct, r.heading ";
$query .= "FROM   tgrams.mt_profile_history as m, $intable as r   ";
$query .= "WHERE  m.tinid=r.tinid  AND date='$fdate' AND r.heading>0 AND r.acct>0 ";
$query .= "GROUP  by r.acct, r.heading, r.tinid  ";  
#print "\n\n$query\n\n";
my $sth = $dbh->prepare($query);
if (!$sth->execute) { print "Error" . $dbh->errstr . "\n"; exit(0); }
while (my $row = $sth->fetchrow_arrayref)
 {  
    ...
    $orgid  = "$fdate$acct$hdd$org$dom$countrycode$city$state$zip$cov";                                        
    $orgid  =  md5_hex($orgid); 
    $orgmd5 =  md5_hex($org); 

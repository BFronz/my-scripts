...
$query  = " select acct, tinid, count(*) from tgrams.contacts ";
$query .= " where (created>=$start and created<=$end) and id>0 and acct>0 and notsent<1 and tinid>''  and test_msg!=1 and other!=50 and sell!=50 ";
...
while (my $row = $sth->fetchrow_arrayref)
 {   
    ...
    $tinid = $$row[1];
    $cnt   = $$row[2];
    print wf "update $log set ca='$cnt' where date='$logtime' and acct='$act' and tinid='$tinid';\n";    
    $act = $tinid = $cnt = "";
    $j++;
 } 
$sth->finish; 

...
$query  = " select m.company, m.acct from thomtnetlogREG14 as r, tgrams.main as m where m.acct=r.acct and r.date in ($rdate)  ";
...
foreach $record (@record)         
 {                               
   ...
   # visitor report     
   $query  = "SELECT org FROM thomtnetlogORGDWIZ   ";
   $query .= "WHERE acct='$acct' AND date in ($rdate) and latitude='0.0001' GROUP BY org ORDER BY  org ";
   #print "$query\n";
   my $sth = $dbh->prepare($query); 
   $b = "0"; 
   if (!$sth->execute) { print "Error" . $dbh->errstr . "<BR>\n"; exit(0); }

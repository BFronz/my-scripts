...
$yy       =  substr($fdate, 0, 2);
$mm       =  substr($fdate, 2, 2);
$table     = "tnetlogORGD15_03_test_all";
$tablefile = $table . ".txt";
...
open(wf2,  ">exclude.txt")  || die (print "Could not open exclude.txt\n");

open(wf3,  ">notreg.txt")  || die (print "Could not open exclude.txt\n");
...
while (!eof(rf))
  {
    ...
    $advv="";
    $squery  = "SELECT adv FROM tgrams.main WHERE acct='$acct'  ";
  ...
  $pre_tinid   = $tinid;
	...
	if($tinid ne "") {
		...
		$ssth->finish;
		#print "$tinid\t $tinidok\n";
                if($tinidok eq "0") {print wf3 "$tinid\n";  }
   	}
	...
	if($badword{$org} ne "") { $block="Y";} else { $block="N";}
  	$hastinidok++;
	...
	$post_tinid   = $tinid;
     ...
     { 
	$post_acct    = $pre_acct;
	$post_visitor = $pre_visitor;
	$post_adv     = $advv;
	$post_isp     = $pre_isp;
	$post_block   = $pre_block;
	$post_tinid   = $pre_tinid;
...
#     if($block ne "Y") { 
     ...
     print  wf "$pre_acct\t$pre_visitor\t$pre_adv\t$pre_isp\t$pre_block\t$pre_tinid\t$post_acct\t$post_visitor\t$post_adv\t$post_isp\t$post_block\t$post_tinid\n";
 


#     } 
#   } 
    $date = $acct = $org = $dom = $country = $city = $state = $zip = $cnt = $orgid = $ip = $naics = $primary_sic = $countrycode = $orgmd5 = $advv= $tinidokna= "";

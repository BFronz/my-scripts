...
elsif($infile =~ /USER_IP_ADDRESSES/ )          { $table="catnav_ip" . $yy; $dontimport=1; }
...
$query = "delete from $table2 where date = '$fdate' ";
my $sth = $dbh->prepare($query);
if (!$sth->execute) { print "Error" . $dbh->errstr . "<BR>\n"; exit(0); }
$sth->finish;
}
...
if($infile =~ /CN_ACTIVITY_MONTHLY/ )       
 { 
  ...
  $sth->finish; 
  close($wf2);
  system("mysqlimport thomas $outfile2");
  

  $query  = "update catnav_summmary$yy ";

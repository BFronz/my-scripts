...
$orgtable = "thomtnetlogORGCATN" .  $yy; 
...
$z=0;
$placeholder=""; 
$query  = " select keyword,trim(name) from browselist where homepage=1 and display=1 and browse=1 and parent=0 group by name";
#$query .= " limit 1 ";
...
while (my $row = $sth->fetchrow_arrayref)
 { 
   ...
   # Now get sub cats 
   $subq ="select keyword, name from browselist where parent='$supernum' and display!='' order by name ";
   ...
   while (my $srow = $subr->fetchrow_arrayref)
    { 
     ...
     $subq2 = "select count(*) as n from browselist where parent=$subnum and display>''";
     ...
     if($count ne 0)
       { 
         ...
         $subq2 = "select keyword, name from browselist where parent=$subnum and display>'' order by name";
         my $subr2 = $dbh->prepare($subq2);
         if (!$subr2->execute) { print "Error" . $tbh->errstr . "\n"; }
         while (my $srow2 = $subr2->fetchrow_arrayref)
          {    
...
foreach $record (@record)
 {  
  ...
  $query  = " SELECT heading FROM browseheads WHERE keyword=$subnum group by heading ";
  # print "$query\n";
  ...
  if($headingct[$subnum] ne 0)
   {
     # Get each adv/free  
     $query  = " select sum(cnt_headin) as comps, sum(adv) as adv  from covprodhd where area='NA' and heading in($heading[$subnum]) ";
     my $sth = $dbh->prepare($query);  
     if (!$sth->execute) { print "Error" . $dbh->errstr . "<BR>\n"; exit(0); }
     while (my $row = $sth->fetchrow_arrayref)
      { 
     ...
     # Get conv
     $query  = "select  sum(cd + cl + ec + em + lw + mi + mt + pc + pv + lc) as conv from $quicklog where date='$fdate' and covflag='t' and heading in ($heading[$subnum]) and acct>0 ";
     my $sth = $dbh->prepare($query);   
     if (!$sth->execute) { print "Error" . $dbh->errstr . "<BR>\n"; exit(0); }
     while (my $row = $sth->fetchrow_arrayref)
      {    

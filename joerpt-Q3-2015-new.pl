...
$outfile = "Conversion-Action-Detail-New-Q3-2015.txt";
system("rm -f $outfile");
...
foreach $record (@record)
{
 ...
 $q .= "FROM tnetlogARTU$yy WHERE acct in ($acctmap) and date in ($fdate) and covflag='t' ";
 PrintQ($q);
 my $sth = $dbh->prepare($q);
 if (!$sth->execute) { print "Error" . $dbh->errstr . "<BR>\n"; exit(0); }
 while (my $row = $sth->fetchrow_arrayref)
...
if(linkscatalog==0) {$linkscatalog>"0";}
 ...
 $q = "SELECT sum(nsv)  AS newsviews, sum(nlw)  AS linksweb,  sum(nsv) + sum(ncc) + sum(nes) + sum(nlw) AS totnews FROM thomnews_conversions$yy WHERE acct IN ($acctmap) AND date in ($fdate)  ";
 PrintQ($q);
 my $sth = $dbh->prepare($q);
 if (!$sth->execute) { print "Error" . $dbh->errstr . "<BR>\n"; exit(0); }
 while (my $row = $sth->fetchrow_arrayref)
 ...
 $adclicks =  $adviews = "0";
 $q = "SELECT sum(BannerClicks) AS adclicks, sum(BannerViews) AS adviews FROM thomnews_ad_cat$yy WHERE AdvertiserCid in ($acctmap) AND date in ($fdate)  AND badimg='N' ";
 PrintQ($q);
 my $sth = $dbh->prepare($q);
 if (!$sth->execute) { print "Error" . $dbh->errstr . "<BR>\n"; exit(0); }
 while (my $row = $sth->fetchrow_arrayref)
 ...
 $q = "SELECT sum(clicks) AS adclicks, sum(views) AS adviews FROM thomtnetlogADviewsServer$yy WHERE acct IN ($acctmap) AND adtype ='pai' AND fdate in ($fdate)  ";
 PrintQ($q);
 my $sth = $dbh->prepare($q);
 if (!$sth->execute) { print "Error" . $dbh->errstr . "<BR>\n"; exit(0); }
 while (my $row = $sth->fetchrow_arrayref)
 ...
 $q = "SELECT sum(clicks) AS adclicks, sum(views) AS adviews FROM thomtnetlogADviewsServer$yy WHERE acct IN ($acctmap) AND adtype ='bad' AND fdate in ($fdate)  ";
 PrintQ($q);
 my $sth = $dbh->prepare($q);
 if (!$sth->execute) { print "Error" . $dbh->errstr . "<BR>\n"; exit(0); }
 while (my $row = $sth->fetchrow_arrayref)
 ...
 $visitor = "0";
 $q = "select count(distinct org) FROM thomtnetlogORGDAllM WHERE acct IN ($acctmap) AND isp='N' AND date in ($fdate)  ";
 PrintQ($q);
 my $sth = $dbh->prepare($q);
 if (!$sth->execute) { print "Error" . $dbh->errstr . "<BR>\n"; exit(0); }
 while (my $row = $sth->fetchrow_arrayref)
 ...
 $caton = $catonrfq = "0";
 $q = "SELECT SUM(totalpageviews) AS n, sum(totalInq) + sum(totalordrfqs) totalInq FROM thomflat_catnav_summmaryM WHERE tgramsid IN ($acctmap) AND date in ($fdate)   ";
 PrintQ($q);
 my $sth = $dbh->prepare($q);
 if (!$sth->execute) { print "Error" . $dbh->errstr . "<BR>\n"; exit(0); }
 while (my $row = $sth->fetchrow_arrayref)
 ...
 $catoff = $catoffrfq = "0";
 $q = "SELECT SUM(totalpageviews) AS n, sum(totalInq) + sum(totalordrfqs) totalInq FROM thomcatnav_summmaryM WHERE tgramsid IN ($acctmap) AND date in ($fdate)   ";
 PrintQ($q); 
 my $sth = $dbh->prepare($q);
 if (!$sth->execute) { print "Error" . $dbh->errstr . "<BR>\n"; exit(0); }
 while (my $row = $sth->fetchrow_arrayref)

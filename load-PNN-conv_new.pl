...
#$file[1]  = "ncc:advPRIDEmailCompany";  # contact company
#$file[1]  = "nes:advPRIDEmailStory";    # email story          
...
while (my $row = $sth->fetchrow_arrayref)
 {
  ...
  print wf "$date\t$acct\t$prid\t$nsv\t$nlw\t$dpub\t$hdl\n";

  $acct = $prid = $nsv = $nlw = $dpub = $hdl  = "";
 }
$sth->finish;
close(wf);

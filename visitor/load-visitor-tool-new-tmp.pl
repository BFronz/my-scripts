...
$query .= "$query .= "employeerange,forbes2k,fortune1k,industry,informationlevel,latitude,longitude,phone,revenuerange,subindustry ";
$query .= "from $table "; 
...
$sth->finish;
system("mysqlimport -r thomas $outfile");
...
$query .= "ip, naics, primary_sic, countrycode, country, ";
$query .= "dunsnum,domestichqdunsnumber,hqdunsnumber,gltdunsnumber,countrycode3,audience,audiencesegment,b2b,employeerange,":
$query .= "forbes2k,fortune1k,industry,informationlevel,latitude,longitude,phone,revenuerange,subindustry,cov ";
$query .= "from $table ";
$query .=" where( org>'' && org!='?') and domain!='?' ";
$query .= "and isp='N' and block='N' ";
$query .= "and heading>0 ";
...
while (my $row = $sth->fetchrow_arrayref)
 { 
   ...
   print wf "$$row[13]\t$$row[14]\t$$row[15]\t$$row[16]\t$$row[17]\t$$row[18]\t$$row[19]\t$$row[20]\t";                    
   print wf "$$row[21]\t$$row[22]\t$$row[23]\t$$row[24]\t$$row[25]\t$$row[26]\t$$row[27]\t$$row[28]\t$$row[29]\t$$row[30]\t$$row[31]\n";                     

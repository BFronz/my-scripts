...
$query  = "SELECT zip, city, state FROM thomregorgs WHERE zip>'0' and city>'' order by zip limit 540, 1000 ";
#$query .= "limit 10";
...
foreach $record (@record)
{
	...
	$query = "UPDATE $table SET city='$city', state='$state' WHERE zip='$zip' AND date in ($dates) AND latitude='0.0001' AND country='united states'  ";
        #print "$query\n\n"; 
	my $sth = $dbh->prepare($query);
	if (!$sth->execute) { print "Error" . $dbh->errstr . "<BR>\n"; exit(0); }
	$sth->finish;

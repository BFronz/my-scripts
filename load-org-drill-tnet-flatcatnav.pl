#!/usr/bin/perl
#
# Loads new Org drill for flat catnav
...
$yy       =  substr($fdate, 0, 2);
$mm       =  substr($fdate, 2, 2);
$table     = "flat_catnav_ORG" . $yy ;
$tablefile = $table . ".txt";
#$file    = "CatFlatVisitors";
$file    = "CatFlatVisitorsCov";
$file    = "CatFlatVisitorscovnewreg";
$infile  = $fyear . "/" . $file . "-" . $fdate . ".txt";
open(rf, "$infile")  || die (print "Could not open $infile\n");



# Get bad and isp orgs - need only bad now
...
$query = "DELETE FROM thomflat_catnav_ORG_temp";
my $sth = $dbh->prepare($query);
if (!$sth->execute) { print "Error" . $dbh->errstr . "<BR>\n"; exit(0); }
$sth->finish;

# load temp table
#open(wf,  ">flat_catnav_ORG_temp.txt")  || die (print "Could not open flat_catnav_ORG_temp.txt\n");
...
while (!eof(rf))
  {
    ...
    ($date,$acct,$org,$cnt,$dom,$city, $state, $zip, $isp, $ip, $naics, $country, $primary_sic, $countrycode, $duns_number, $domestichqdunsnumber, $hqdunsnumber, $gltdunsnumber, $countrycode3, $audience, $audiencesegment, $b2b, $employee_range, $forbes2k, $fortune1k, $industry, $informationlevel, $latitude, $longitude, $phone, $revenue_range, $subindustry, $cov, $tinid)  = split(/\t/, $instr);
     ...
     #if($tinid ne "-") { print "$org\tcov: $cov\ttinid: $tinid\n";}
     #sleep(1);
    ...
    $orgid  = "$date$acct$org$dom$countrycode$city$state$zip";
    $orgid  =  md5_hex($orgid);
    $orgmd5 =  md5_hex($org);
  ...
  if($org  =~ /fuck/ || $org =~ /asshole/)  { $block="Y"; }
   ...
   if($org ne "?"  &&  $org ne ""  &&  $org ne "#NAME?"  &&  $org ne "unknown"  &&  $org ne "-"  && $org ne "no company")  {
    ...
    $block=$b2b=$employee_range=$forbes2k=$fortune1k=$industry=$informationlevel=$latitude=$longitude=$phone=$revenue_range=$subindustry=$cov=$tinid=$tinidok="";
...
$query .= "FROM thomflat_catnav_ORG_temp WHERE  org!='-'   ";
#my $sth = $dbh->prepare($query);
#if (!$sth->execute) { print "Error" . $dbh->errstr . "<BR>\n"; exit(0); }
#$sth->finish;

...
$yy        =  substr($fdate, 0, 2);
$mm        =  substr($fdate, 2, 2);
$table     = "tnetlogORGPS" . $yy . "_" . $mm ;
$tablefile = $table . ".txt";
$file      = "advPSOrgDrillCov";
...
$query = "DELETE FROM thomtnetlogORGPS_temp";
my $sth = $dbh->prepare($query);
if (!$sth->execute) { print "Error" . $dbh->errstr . "<BR>\n"; exit(0); }
$sth->finish;
...
# load temp table
#open(wf,  ">tnetlogORGPS_temp.txt")  || die (print "Could not open $outfile\n");
print "$infile\n";
...
while (!eof(rf))
  {
   ...
   if($org ne "?"  &&  $org ne ""  &&  $org ne "#NAME?"  &&  $org ne "unknown"  &&  $org ne "-" &&  $org ne "no company") {
    ...
    $date = $acct = $org = $dom = $country = $city = $state = $zip = $cnt = $orgid = $ip = $isp = $naics = $primary_sic = $countrycode = $orgmd5 = "";
...
$query .= "FROM thomtnetlogORGPS_temp WHERE  org!='-'   ";
#my $sth = $dbh->prepare($query);
#if (!$sth->execute) { print "Error" . $dbh->errstr . "<BR>\n"; exit(0); }
#$sth->finish;
